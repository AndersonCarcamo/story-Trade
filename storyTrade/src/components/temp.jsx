...
const likeBook = async (bookId, userId) => {
  try {
    const likerId = await AsyncStorage.getItem('userId');  // Obtener el id del usuario desde AsyncStorage
    if (!likerId) {
...
const BookDetails = ({ book, goBack }) => {
  ...
  const handleLike = async () => {
    ...
    setHasLiked(!hasLiked);  // Alternar el estado de "like"
  };

  const viewProfile = () => {
    // Implementar lógica para ver perfil del usuario
  };
