...
const fetchBookImages = async (books) => {
  const updatedBooks = await Promise.all(
    books.map(async (book) => {
      if (!book.book_info || !book.book_info.image) {
        book.imageUri = defaultImage;
      } else {
        try {
          const imageUrl = `http://localhost:3000/get?mediaType=images&fileName=${book.book_info.image}`;
...
const SearchBook = ({ books, search, handleSearch, viewDetails }) => {
    ...
    const performSearch = async () => {
      if (search) {
        try {
          ...
          console.log('Filtered books after search:', updatedResults);
        } catch (error) {
  ...
  useEffect(() => {
    ...
    setGroupedBooks(groupBooksByCategory(filteredBooks));
